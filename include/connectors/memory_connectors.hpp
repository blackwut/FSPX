...
template <int W, int DEPTH_IN, int BURST_LENGHT = 4096 / (W / 8)>
...
prepare_burst_wide:
    ...
    #pragma HLS LOOP_TRIPCOUNT min = 1 max = 1024
        ...
        if (bc + 1 == BURST_LENGHT) {
            // signal a complete burst
            burst_size.write(BURST_LENGHT);
            bc = 0;
        } else {
            bc = bc + 1;
        }
...
template <typename T, int DEPTH_IN, int W, int BURST_LENGHT = 4096 / (W / 8)>
void prepare_burst(
...
prepare_burst:
    ...
    remaining_items:
        ...
        #pragma HLS unroll
            if (j >= i) {
                tmp.range(T_BITS * (i + 1) - 1, T_BITS * i) = 0;
            }
        }
        out.write(tmp);
        bc = bc + 1;
...
template <int W, int BURST_LENGHT = 4096 / (W / 8)>
void burst_write(
...
burst_write:
    ...
    one_burst:
        for (int j = 0; j < bs; j++) {
        #pragma HLS pipeline II = 1
        #pragma HLS LOOP_TRIPCOUNT min = 1 max = BURST_LENGHT
            out[i * BURST_LENGHT + j] = in.read();
        }
        i++;
        bs = burst_size.read();
        wc += bs;
...
#pragma HLS DATAFLOW
    ...
    constexpr int fifo_buf = 2 * BURST_LENGHT;
