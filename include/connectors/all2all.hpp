...
// TODO: Add implementation without constexpr if for each operator.
//       They should be separate functions with different names because of
//       the different signatures of the functors.
...
// NOTE: the replication of operators cannot be done without constexpr if because
//       during the compilation the same functor is passed to all the operator functions
//       (fx::Map, fx::Filter, fx::FlatMap). Each of the operator function has
//       a different signature for the functor, and so the compilation fails.
//       With constexpr if, the compiler removes the code that is not needed,
//       and so the compilation succeeds as it does not need to compile the
//       functions that are not used.

template <typename T>
auto default_key_extractor_t = [](const T & r){ return int(0); };
auto default_key_generator_t = [](const int index){ return int(0); };
