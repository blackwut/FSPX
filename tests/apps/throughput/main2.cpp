...
const int pc_source[2] = {PC_NAME(0), PC_NAME(1)};
const int pc_sink[2] = {PC_NAME(29), PC_NAME(30)};
const int pc_sink_data = PC_NAME(31);
...
struct Source_Execution
{
    ...
    , i(i)
    {
        ...
        kernel = ocl.createKernel("memory_reader:{memory_reader_" + std::to_string(source_idx + 1) + "}");
        ...
        batch_ext.param = 0;
        batch_ext.flags = pc_source[source_idx];
...
struct Source
{
    ...
    , running_executions()
    {
        ...
        for (size_t n = 0; n < number_of_buffers; ++n) {
            running_executions.push_back(new Source_Execution<T>(ocl, queue, max_batch_size, source_idx, n));
        }
    }
...
template <typename T>
    struct Sink_Execution {
        ...
        , count_h(0)
        {
            ...
            kernel = ocl.createKernel("memory_writer:{memory_writer_" + std::to_string(sink_idx + 1) + "}");
            ...
            batch_ext.obj = batch_h;
            batch_ext.param = 0;
            batch_ext.flags = pc_sink[sink_idx];
...
struct Sink
{
    ...
    , sink_idx(sink_idx)
    {
        ...
        *eos = 0;
        ...
        for (size_t n = 0; n < number_of_buffers; ++n) {
            waiting_executions.push_back(new Sink_Execution<T>(ocl, queue, max_batch_size, &eos_d, sink_idx));
        }
...
{
    fx::Sink<record_t> sink(ocl, batch_size, n, idx);
